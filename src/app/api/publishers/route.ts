import { createPublisherSchema } from '@/features/publishers/schema'
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (responseType) {
      const publishersWithoutPagination = await prisma.publisher.findMany({
        select: {
          id: true,
          name: true,
        },
      ...
      return ApiResponseHandler.success(publishersWithoutPagination)
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validationResult = createPublisherSchema.safeParse(body)
    ...
    return ApiResponseHandler.success(publisher, 'ناشر با موفقیت ایجاد شد', 201)
  } catch (error) {
    return ApiResponseHandler.internalError(undefined, error)
  }
}
